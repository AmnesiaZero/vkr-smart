...
@section('content')
    <div class="list">
        ...
        <form enctype="multipart/form-data" id="news_form" action="{{ route('news.store') }}" data-action-index="{{ route('news.index') }}" method="POST">
            @csrf
            <div class="post">
                <div class="form-group">
                    <label for="title" style="font-size:14px;">Заголовок</label>
                    <input id="title" type="text" name="title" class="form-control" value="@if(isset($item->title)) {{$item->title}} @endif">
                </div>
                <div class="form-group">
                    <label for="annotaion" style="font-size:14px;">Анонс новости</label>
                    <textarea id="annotaion" name="annotaion" rows="4" class="form-control"> @if(isset($item->annotaion)) {{$item->annotaion}} @endif</textarea>
                </div>
                <div class="form-group">
                    <label for="text" style="font-size:14px;">Текст новости</label>
                    <textarea id="text" name="text" rows="12" class="form-control editor">@if(isset($item->text)) {{$item->text}} @endif</textarea>
                </div>
                <div class="form-group">
                    <label for="preview_image">Изображение новости</label>
                    <div class="input-group">
                        <input id="preview_image" type="text" value="@if(isset($item->preview_name)) {{$item->preview_name}} @endif" class="form-control"
                ...
                <div class="form-group">
                    ...
                    <input id="publication_date" type="text" name="publication_date" class="form-control datepicker" value="@if(isset($item->publication_date)) {{$item->publication_date}} @endif">
                </div>
                <div class="form-group">
                    <label for="published" style="font-size:14px;">Параметры публикации</label>
                    <select name="published" class="form-control" id="published">
                        <option value="0" @if(isset($item->published) and $item->published==0) selected @endif>Не опубликован</option>
                        <option value="1" @if(isset($item->published) and $item->published==1) selected @endif>Опубликован</option>
                    </select>
                </div>
                <div class="form-group">
