<tr id="rowID_{{ $item->id }}" class="rows @if($item->deleted_at) deleted @endif">
	...
	<td class="actions text-center">
		@if($item->deleted_at)
			<a href="{{route('departments.restore',['id' => $item->id])}}" id="status_{{ $item->id }}" data-item-id="{{ $item->id }}" class="restore d-inline mr-2">
                <i id="stat_{{ $item->id }}" class="icon fas fa-trash-restore"></i>
            </a>
			<a href="{{route('departments.destroy',['id' => $item->id])}}" data-item-id="{{ $item->id }}" class="delete d-inline">
		...
		@else
            ...
            <a href="{{route('departments.view.delete',['id' => $item->id])}}" data-item-id="{{ $item->id }}" class="destroy d-inline">
                <i class="icon far fa-trash-alt"></i>
            </a>
		@endif
	</td>
